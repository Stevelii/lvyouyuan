...
function normalizeHomePageContent(input = {}, existing = createDefaultHomePageContent()) {
  ...
  return {
    ...
    contactEyebrow: String(input.contactEyebrow ?? existing.contactEyebrow ?? defaults.contactEyebrow).trim(),
    contactTitle: String(input.contactTitle ?? existing.contactTitle ?? defaults.contactTitle).trim(),
    contactDescription: String(input.contactDescription ?? existing.contactDescription ?? defaults.contactDescription).trim(),
    contactEmail: String(input.contactEmail ?? existing.contactEmail ?? defaults.contactEmail).trim(),
    contactPhone: String(input.contactPhone ?? existing.contactPhone ?? defaults.contactPhone).trim(),
    contactWechatLabel: String(input.contactWechatLabel ?? existing.contactWechatLabel ?? defaults.contactWechatLabel).trim(),
    contactQrImage: String(input.contactQrImage ?? existing.contactQrImage ?? defaults.contactQrImage).trim(),
